...
- 注释！！、注释！！、注释！！ 必须tmd要加注释
  ...
  - 代码`模块化` 不相关的两段代码，空行隔开
    ```java
       // 第一步：查询数据库
       Entity entity = service.getById(1);
       if (entity != null) { xxx }
    ...
    ```
- 定期重构代码
  - 随着日后编程水平提高，你会发现之前写的代码简直惨不忍睹
    这时候就有必要进行重构以保持代码的健壮
- 常量分为三大类，全局常量、业务常量、RedisKey常量
  - 全局常量：全局常量顾名思义就是所有模块通用的常量，例如：`public static final Integer SUCCESS = 0`
  - 业务常量：当前业务模块下所用到的常量，例如：`public static final Integer USER_DEFAULT_PWD = 123456`
  - RedisKey常量：又分全局和业务常量，主要是存放RedisKey前缀， 例如：`public static final String CAPTCHA_KEY = "system:captcha"`

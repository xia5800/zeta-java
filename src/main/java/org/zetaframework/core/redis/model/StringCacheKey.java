 ...
 * @author gcc
 */
public interface StringCacheKey extends CacheKey {

    default ValueOperations<String, Object> getValueOps() {
     ...
     * @return T      缓存值 查不到则返回null，查得到则返回对应的对象
     */
    default <T> T get(Object suffix) {
        String key = CacheKeyExt.buildKey(getPrefix(), suffix);
        return CacheKeyExt.returnVal((T) getValueOps().get(key));
    }
    ...
    default Boolean delete(Object suffix) {
        return getRedisTemplate().delete(CacheKeyExt.buildKey(getPrefix(), suffix));
    }
